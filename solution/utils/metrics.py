...
def compute_auprc(logits, labels):
    ...
    for c in range(N_CLASSES):
        ...
        prods_c = logits.take([c], axis=1).ravel()
...
def compute_klue_re_leaderboard(eval_pred):
    ...
    # Compute micro F1
    micro_f1 = compute_micro_f1(logits, labels, label_indices)
    # Compute AURPC
    auprc = compute_auprc(logits, labels)
    # Compute confusion matrix
    cm_fig = get_confusion_matrix(logits, labels)
